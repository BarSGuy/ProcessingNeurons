...
def register_hooks(cfg, model, H_W_dim=16):
    ...
    def get_activation(name):
        def hook(model, input, output):
            ...
            if H_W_dim == 8:
                found_tensor = True
                if output.shape[2:] == torch.Size([32, 32]): # 0_, 1_, 2_
                    output = F.max_pool2d(output, kernel_size=2, stride=4)
                    new_tensor = output
                elif output.shape[2:] == torch.Size([16, 16]): # 3_, 4_
                    output = F.max_pool2d(output, kernel_size=2, stride=2)
                    new_tensor = output
                elif output.shape[2:] == torch.Size([4, 4]): # 6_
                    new_tensor = torch.zeros(1, 256, 8, 8)
                    new_tensor[:, :, 2:6, 2:6] = output
                    # output = new_tensor
                elif output.shape[2:] == torch.Size([2, 2]): # 7_
                    new_tensor = torch.zeros(1, 512, 8, 8)
                    new_tensor[:, :, 3:5, 3:5] = output
                    # output = new_tensor
                elif output.shape[2:] == torch.Size([1, 1]): # 8_
                    new_tensor = torch.zeros(1, 512, 8, 8)
                    new_tensor[:, :, 2, 2] = output[:, :, 0, 0]
                    new_tensor[:, :, 3, 3] = output[:, :, 0, 0]
                    new_tensor[:, :, 4, 4] = output[:, :, 0, 0]
                    new_tensor[:, :, 5, 5] = output[:, :, 0, 0]
                    
                    # output = new_tensor
                elif output.shape[1:] == torch.Size([10]): # 9_
                    new_tensor = torch.zeros(1, 10, 8, 8)
                    new_tensor[:, :, 2, 2] = output
                    new_tensor[:, :, 3, 3] = output
                    new_tensor[:, :, 4, 4] = output
                    new_tensor[:, :, 5, 5] = output
                else:
                    found_tensor = False
            elif H_W_dim == 16:
                found_tensor = True
                if output.shape[2:] == torch.Size([32, 32]): # 0_, 1_, 2_
                    ...
                    new_tensor = output
                elif output.shape[2:] == torch.Size([8, 8]): # 6_
                    new_tensor = F.interpolate(
                        output, size=(H_W_dim, H_W_dim), mode='nearest')
                elif output.shape[2:] == torch.Size([4, 4]): # 6_
                    new_tensor = F.interpolate(
                        output, size=(H_W_dim, H_W_dim), mode='nearest')
                elif output.shape[2:] == torch.Size([2, 2]): # 7_
                    ...
                    output = output.reshape(1, 10, 1, 1)
                    new_tensor = F.interpolate(
                        output, size=(H_W_dim, H_W_dim), mode='nearest')
                else:
                    found_tensor = False
...
def activation_datasets(cfg, model_symmetry = "Cn"):
    original_model = resnet18(pretrained=True).to(cfg.general.device)
    ...
    cifar10_test_dataloader = DataLoader(cifar10_test_dataset,
        batch_size=1,
        shuffle=False,
        num_workers=4
    )    
    ...
    activations_path = './experiments/activations.pt'
    correctness_labels_path = './experiments/correctness_labels.pt'
    if os.path.exists(activations_path):
        ...
        with torch.no_grad():
            ...
            counter = 0 
            for inputs, labels in tqdm(cifar10_test_dataloader):
        ...
        def save_activations(activations, correctness_labels, path):
            torch.save(activations, os.path.join(path, 'activations.pt'))
            torch.save(correctness_labels, os.path.join(path, 'correctness_labels.pt'))
...
def get_tmp_scale_baseline(cfg, results_dict, activations_loader_test, temperature_scaling_model):
    ...
    for batch in tqdm(activations_loader_test):
        x, y = batch[0], batch[1]
        model_pred = x[:, -10:, 4, 4].to(cfg.general.device)
